...
export class Parser {
    ...
    emit(): string {
        ...
        result = result.concat(this.topLevel.reduce((out: string, obj: dom.TopLevelDeclaration) => {
            // Filter out unsupported kinds that dts-dom can't handle
            const unsupportedKinds = ['property', 'member'];
            if (unsupportedKinds.includes((obj as any).kind)) {
                console.log(`Skipping top-level object with unsupported kind: ${(obj as any).kind} - ${(obj as any).name || 'unnamed'}`);
                return out;
            }
            try {
                return out + dom.emit(obj);
            } catch (err) {
                console.error(`Error emitting object:`, obj);
        ...
        for (let doclet of docs)
        {
            ...
            else
            {
                ...
                if (!parent)
                {
                    ...
                    continue; // Skip this doclet if parent is not found
                }

                if (!(parent as any).kind)
                {
                    console.log(`PARENT KIND WARNING: ${doclet.longname} in ${doclet.meta.filename}@${doclet.meta.lineno} has parent '${doclet.memberof}' that has no kind property.`);
                }
